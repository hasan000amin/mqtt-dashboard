...
mqttClient.on("message", (topic, message) => {
  try {
    ...
    console.log(
      `[${new Date().toLocaleTimeString()}] Topic: ${topic} | Data: ${messageStr.substring(0, 100)}`,
    );
  } catch (e) {
    ...
    if (data.length > 0 && data[0].suhu !== undefined) {
      ...
      if (suhuValues.length > 0) {
        stats[topic] = {
          ...
          kelembapan_avg:
            kelembapanValues.length > 0
              ? kelembapanValues.reduce((a, b) => a + b, 0) /
                kelembapanValues.length
              : null,
          kelembapan_min:
            kelembapanValues.length > 0 ? Math.min(...kelembapanValues) : null,
          kelembapan_max:
            kelembapanValues.length > 0 ? Math.max(...kelembapanValues) : null,
        };
      }
    }
  }
